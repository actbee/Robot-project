...
boolean success;
 Minim voice;
 AudioInput myvoice;
balls_manager myballmanager;

void setup() {
  fullScreen(SPAN);
  background(200);
  frameRate(60);
  voice=new Minim(this);
  myvoice=voice.getLineIn(Minim.MONO);
 ...
 // success=true;
  ...
  //myballmanager=new balls_manager(unitx*2,unitx*4,unity*3.5,unity*5.5);
...
class ball{
  ...
  boolean dead;
  float addr;
  ball(float xlow,float xhigh,float ylow,float yhigh){
    addr=0;
    location=new PVector(random(xlow,xhigh),random(ylow,yhigh));
    lifespan=255;
    originr=(float)random(10,30);
    v=new PVector(random(-5,5),random(-5,5));
    a=new PVector(random(-3,3),random(-3,3));
    dead=false;
  }
  void update(float xlow,float xhigh,float ylow,float yhigh){
    addr=abs(myvoice.mix.get(0));
    r=1000*addr+originr;
    checkedge(xlow,xhigh,ylow,yhigh);
   // v.add(a);
    v.limit(vmax);
    location.add(v);
  }
  void checkedge(float xlow,float xhigh,float ylow,float yhigh){
    if(location.x<xhigh-20||location.x>xlow+20){
     // v.x*=-0.8;
       v.x*=-1;
    }
    if(location.y<yhigh-20||location.y>ylow+20){
      // v.y*=-0.8;
      v.y*=-1;
    }
  }
  void render(float xlow,float xhigh,float ylow,float yhigh){
    update(xlow,xhigh,ylow,yhigh);
    noStroke();
    fill(50);
...
class balls_manager{
  ...
  int num;
  balls_manager(float x1,float x2,float y1,float y2){
    xlow=x1;
    xhigh=x2;
    ylow=y1;
    yhigh=y2;
    num=20;
    for(int i=0;i<num;i++){
      balls.add(new ball(xlow,xhigh,ylow,yhigh));
    }
  }
  void render(){
    for(int i=0;i<num;i++){
      balls.get(i).render(xlow,xhigh,ylow,yhigh);
    }
  }
}